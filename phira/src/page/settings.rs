...
impl ChartList {
    ...
    pub fn touch(&mut self, touch: &Touch, t: f32) -> Result<Option<bool>> {
        ...
        if self.combo_tip_btn.touch(touch, t) {
            request_input("combo_tip", &config.combo_tip);
            return Ok(Some(true));
        }
        Ok(None)
    }
    ...
    pub fn render(&mut self, ui: &mut Ui, r: Rect, t: f32) -> (f32, f32) {
        ...
        item! {
            render_title(ui, tl!("item-combo-tip"), Some(tl!("item-combo-tip-sub")));
            self.combo_tip_btn.render_text(ui, rr, t, &config.combo_tip, 0.4, false);
        }
