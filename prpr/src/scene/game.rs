...
impl GameScene {
    ...
    fn ui(&mut self, ui: &mut Ui, tm: &mut TimeManager) -> Result<()> {
        ...
        ui.alpha(res.alpha, |ui| {
            ...
            // score
            let h = 0.07;
            let score_top = top + eps * 2.2 - (1. - p) * 0.4 + 0.005;
            let score = format!("{:07}", self.judge.score());
            let ct = ui.text(&score).size(1.25).set_uh(true).measure_using(&PGR_FONT).center();
            self.chart
            ...
            self.chart
                .with_element(ui, res, UIElement::Pause, Some((pause_center.x, pause_center.y)), |ui, c| {
                    let mut r = Rect::new(pause_center.x - pause_w, pause_center.y - pause_h / 2., pause_w, pause_h);
                    ui.fill_rect(r, c);
                    r.x += pause_w * 2.;
                    ui.fill_rect(r, c);
                });
            if self.judge.combo() >= 3 {
                ...
                self.chart
                    .with_element(ui, res, UIElement::Combo, Some((0., combo_top + unit_h * 0.2)), |ui, c| {
                        ui.text(&res.config.combo_tip) //(if res.config.autoplay() { "AUTOPLAY" } else { "COMBO" })
                            .pos(0., combo_top)
                            .anchor(0.5, 0.)
                            .size(0.6)
                            .color(c)
            ...
            let lf = -1. + margin + 0.005;
            let bt = -top - eps * 2.8 + (1. - p) * 0.4 - 0.005;
            ...
            ui.fill_rect(Rect::new(-1., top, dest, height), Color{r:0.6, g:0.6, b:0.6, a:1.});
            ui.fill_rect(Rect::new(-1. + dest - hw, top, hw, height), WHITE);
        });
        Ok(())
    }
    ...
    fn overlay_ui(&mut self, ui: &mut Ui, tm: &mut TimeManager) -> Result<()> {
        ...
        if tm.paused() {
            ...
            ui.fill_rect(r, (*res.icon_pause_retry, r.feather(0.01), ScaleType::Fit, if no_retry { semi_white(res.alpha * 0.6) } else { c }));
            draw_texture_ex(
                *res.icon_pause_resume,
                s + w,
                -s + o,
                c,
                DrawTextureParams {
